...
- **Sorting Module**: `SortingState`/`SortingItem`, `SortOp`, `IBarVisualizer`, `SortingAlgorithmCatalog`, and `SortingRunner` demonstrate the pipeline with bubble, selection, merge, and stable insertion sort implementations.
...
3. Consumers either pass an `ISortingAlgorithm` directly or call `ExecuteAsync("merge-sort", state, token)`.
...
3. Run an algorithm via descriptor.
   ...
   var sorted = await runner.ExecuteAsync("merge-sort", state, CancellationToken.None);
   ```
